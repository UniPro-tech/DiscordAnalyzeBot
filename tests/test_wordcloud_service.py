...
def test_reset_learning_state_for_hybrid_uses_mongo_meta_and_clickhouse_learning():
    class _MongoMetaCollection:
        def __init__(self):
            self.deleted_keys = []

        def delete_one(self, query):
            self.deleted_keys.append(query.get("_id"))

    class _MongoMessagesCollection:
        def __init__(self):
            self.docs = [{"_id": 1, "tokens": ["tok"]}]
    ...
    class _MongoDB:
        ...
        def __init__(self):
            self.meta = _MongoMetaCollection()
            self.messages = _MongoMessagesCollection()
    ...
    class _ClickHouseDB:
        ...
        def __init__(self):
            self.commands = []
            self.rows = [{"message_id": "1", "tokens": ["tok"]}]
        ...
        def query_scalar(self, _query, _parameters=None):
            return 1
    ...
    class _HybridDB:
        ...
        def __init__(self):
            self.db_mongo = _MongoDB()
            self.db_clickhouse = _ClickHouseDB()

    db = _HybridDB()

    reset_learning_state(db)

    assert db.db_clickhouse.rows[0]["tokens"] == []
    assert set(db.db_mongo.meta.deleted_keys) == {"last_learn_cursor", "last_learn_id"}
    assert any("TRUNCATE TABLE IF EXISTS unigrams" in q for q, _ in db.db_clickhouse.commands)
    assert any("TRUNCATE TABLE IF EXISTS ngrams" in q for q, _ in db.db_clickhouse.commands)
    assert any("TRUNCATE TABLE IF EXISTS compounds" in q for q, _ in db.db_clickhouse.commands)